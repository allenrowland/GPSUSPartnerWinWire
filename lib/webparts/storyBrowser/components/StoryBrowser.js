...
var StoryBrowser = /** @class */ (function (_super) {
    ...
    function StoryBrowser(props, state) {
        ...
        _this.onSubmissionTypeFilterChange = function (cfilter, isActive) {
            var currentFilters = _this.state._storyBrowserStateFilters.filter(function (item) { return item !== cfilter; });
            if (isActive) {
    ...
    StoryBrowser.prototype.render = function () {
        ...
        var featuredStories = filteredStories.filter(function (item) { return item.Featured == "Yes"; }).slice(0, 3);
        return (React.createElement("section", { id: "storyBrowser", className: styles.storyBrowser, onClick: this.hideFilters },
            React.createElement("div", { id: "filters", className: styles.filtersContainer },
                    ...
                    React.createElement("div", { className: styles.filters },
                        ...
                        React.createElement("div", { className: styles.filterOptions },
                            React.createElement("div", null,
                                ...
                                React.createElement(FilterGroup, { title: 'File Type', groupName: 'LinkType', filterOptions: this.props.filters.filter(function (item) { return item.Field == 'LinkType'; }), onChange: this.onSubmissionTypeFilterChange, activeFilters: this.state._storyBrowserStateFilters })),
                            React.createElement("div", null,
                ...
                React.createElement("div", { className: styles.filtersSearch },
                    React.createElement("input", { style: inputStyle, placeholder: "Search by keyword", type: "text", name: "namesearch", onChange: this.handleSearchChange, value: this.state._storyBrowserStateSearchTerm != '' ? this.state._storyBrowserStateSearchTerm : null }),
                    " ",
    ...
    StoryBrowser.prototype.filterStories = function (stories) {
        ...
        if (this.state._storyBrowserStateSearchTerm != null && this.state._storyBrowserStateSearchTerm != '') {
            ...
            stories.forEach(function (storyItem) {
                ...
                var tags = typeof (storyItem['Tags']) == "string" ? ['Tags'] : (Object)(storyItem['Tags']);
                for (var i in tags) {
                    if (tags[i].toLowerCase().indexOf((_this.state._storyBrowserStateSearchTerm).toLowerCase()) > -1) {
                        pushStory = true;
                    }
                }
                if (storyItem.Title.toLowerCase().indexOf((_this.state._storyBrowserStateSearchTerm).toLowerCase()) > -1) {
                    pushStory = true;
                }
                if (pushStory) {
        ...
        if (this.state._storyBrowserStateFilters.length > 0) {
            ...
            stories.forEach(function (storyItem) {
                ...
                var filterMatches = 0; //(AND)
                cfilters_1.forEach(function (filterItem) {
                    var cfilter = filterItem.split('|')[0];
                ...
                //if(pushStory){ (OR)
                if (filterMatches == cfilters_1.length) { //(AND)
    ...
    StoryBrowser.prototype.storyCard = function (item) {
        return (React.createElement("div", { className: styles.item },
            React.createElement("div", { className: styles.image },
                React.createElement("img", { src: item.Image['serverRelativeUrl'], alt: item.Title })),
