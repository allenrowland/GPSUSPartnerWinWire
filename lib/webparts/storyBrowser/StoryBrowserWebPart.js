...
var StoryBrowserWebPart = /** @class */ (function (_super) {
    ...
    StoryBrowserWebPart.prototype._setfieldChoices = function () {
        sp.setup({
            spfxContext: this.context
        });
        var list = sp.web.lists.getById('f5b9c35f-13e1-4444-bb9a-5a5556159c16');
        var tags = list.fields.getByInternalNameOrTitle('Tags');
        tags.select('Choices').get().then(function (options) {
            //RENDER FILTERS
            console.log(options['Choices']);
        });
        var industry = list.fields.getByInternalNameOrTitle('Industry');
        industry.select('Choices').get().then(function (options) {
            //RENDER FILTERS
            console.log(options['Choices']);
        });
        var partnerType = list.fields.getByInternalNameOrTitle('PartnerType');
        partnerType.select('Choices').get().then(function (options) {
            //RENDER FILTERS
            console.log(options['Choices']);
        });
        var solutionArea = list.fields.getByInternalNameOrTitle('SolutionArea');
        solutionArea.select('Choices').get().then(function (options) {
            //RENDER FILTERS
            console.log(options['Choices']);
        });
        var storyType = list.fields.getByInternalNameOrTitle('StoryType');
        storyType.select('Choices').get().then(function (options) {
            //RENDER FILTERS
            console.log(options['Choices']);
        });
    };
    StoryBrowserWebPart.prototype._getSearchData = function (Industry) {
        ...
        return __awaiter(this, void 0, void 0, function () {
            ...
            return __generator(this, function (_a) {
                ...
                return [2 /*return*/, sp.web.lists.getById('f5b9c35f-13e1-4444-bb9a-5a5556159c16').renderListDataAsStream({
